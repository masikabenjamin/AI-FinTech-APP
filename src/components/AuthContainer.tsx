...
export const AuthContainer: React.FC<AuthContainerProps> = ({
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Fallback for browser autofill patterns when React state doesn't receive input events
    let nameVal = name;
    let emailVal = email;
    let passwordVal = password;
    let confirmVal = confirmPassword;

    const nameInput = document.getElementById('register-name-field') as HTMLInputElement;
    const emailInput = document.getElementById('register-email-field') as HTMLInputElement;
    ...
    if (!nameVal.trim()) errors.name = 'Full identity legal name is required.';
    if (!emailVal.trim()) {
    ...
    setIsLoading(true);
    try {
      await registerCustomer(nameVal, emailVal, passwordVal);
      setSuccessMsg('Sovereign client credentials listed. Logging you in...');
      
      // Auto login customer immediately
      setTimeout(async () => {
        try {
          const authData = await loginUser(emailVal, passwordVal);
          onAuthSuccess(authData.sessionId, authData.user);
        } catch (loginErr: any) {
  ...
  const handleLogin = async (e: React.FormEvent, isAdmin: boolean) => {
    ...
    // Fallback for browser/Chrome credential autofill when React state is left un-triggered
    let emailVal = email;
    let passwordVal = password;

    const emailId = isAdmin ? 'admin-email-field' : 'customer-email-field';
    const passwordId = isAdmin ? 'admin-password-field' : 'customer-password-field';
